...
%
% Modified by Ben Rodanski (ilvento44@gmail.com)
% 2020-10-25:  Using Windows GUI to select a netlist file
%              Use fprintf() instead of disp(sprintf(..))
%              Replaced textread() by textscan()
%              Replaced str2num() by str2double() and a few minor changes
%              suggested by Matlab 2020a.
%
clear variables
close all
...
%
% Get the netlist
[fname,pathname] = uigetfile({'*.cir'},'Get the NetList');
if fname == 0
   return
end
...
Y=cell(n,n);  [Y{:}]=deal('0');    % G is nxn filled with '0'
B=cell(n,m);  [B{:}]=deal('0');
C=cell(m,n);  [C{:}]=deal('0');
D=cell(m,m);  [D{:}]=deal('0');
i=cell(n,1);  [i{:}]=deal('0');
e=cell(m,1);  [e{:}]=deal('0');
j=cell(m,1);  [j{:}]=deal('0');
v=compose('v_%d',(1:n)');          % v is filled with node voltage labels
...
for k1=1:nLines
    ...
    switch Name{k1}(1)
        ...
        case {'R', 'L', 'C'} % RXXXXXXX N1 N2 VALUE
            switch Name{k1}(1)  % Find admittance for each element type.
                case 'R'
                    y = ['1/' Name{k1}];
                case 'L'
                    y = ['1/(s*' Name{k1} ')'];
            ...
            % Here we fill in Y array by adding admittances.
            % The procedure is slightly different if one of the nodes is
            % ground, so check for thos accordingly.
            if (n1==0)
                Y{n2,n2} = [Y{n2,n2} ' + ' y];  % Add admittance.
            elseif (n2==0)
                Y{n1,n1} = [Y{n1,n1} ' + ' y];  % Add admittance.
            else
                Y{n1,n1} = [Y{n1,n1} ' + ' y];  % Add admittance.
                Y{n2,n2} = [Y{n2,n2} ' + ' y];  % Add admittance.
                Y{n1,n2} = [Y{n1,n2} ' - ' y];  % Sub admittance.
                Y{n2,n1} = [Y{n2,n1} ' - ' y];  % Sub admittance.
            end
        ...
        % Op amp
        case 'O'  % OXXXXXXX N1 N2 N3 VALUE  (N1=+, N2=-, N3=Vout)
...
A = str2sym([Y B; C D]); %Create and display A matrix
fprintf('\nThe A matrix: \n');
disp(A);
